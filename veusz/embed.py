...
class Embedded:
    ...
    def __init__(
        self, name="Veusz", copyof=None, hidden=False, compatlevel=0, debug=False
    ):
    ...
    def makeRemoteProcess(cls, debug):
            ...
            if not findexe:
                try:
                    ...
                    findexe = os.path.join(
                        os.environ["ProgramFiles"], "Veusz", "veusz.exe"
                    )
                except KeyError:
                    pass
        ...
        if sys.platform == "darwin":
            ...
            if findbundle:
                ...
                possiblecommands += [["/Applications/Veusz.app/Contents/MacOS/Veusz"]]
                possiblecommands += [
                    [
                        os.path.expanduser(
