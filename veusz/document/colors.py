...
class ColorModel(qt.QAbstractListModel):
    ...
    def flags(self, index):
        ...
        return qt.QAbstractListModel.flags(self, index) | qt.Qt.ItemFlag.ItemIsEditable

    def setData(self, index, value, role):
        if (
            role == qt.Qt.ItemDataRole.EditRole
            or role == qt.Qt.ItemDataRole.DisplayRole
        ):
            row = index.row()
    ...
    def insertRows(self, row, count, parent):
        ...
        self.colorlist = self.colorlist[:row] + [""] * count + self.colorlist[row:]
        self.endInsertRows()
        return True
    ...
    def removeRows(self, row, count, parent):
        ...
        self.colorlist = self.colorlist[:row] + self.colorlist[row + count :]
        self.endRemoveRows()
        return True
    ...
    def updateColorList(self):
        ...
        newcols = (
            self.colors.defaultnames
            + self.colors.definednames
            + self.colors.themenames
            + self.xtranames
        )
        newset = set(newcols)
        ...
        if len(newcols) < len(curcols):
            self.beginRemoveRows(qt.QModelIndex(), len(newcols), len(curcols) - 1)
