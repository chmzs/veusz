...
class DBusInterface(vzdbus.Object):
    ...
    @vzdbus.method(dbus_interface=interface, in_signature="sa{sv}")
    def Action(self, action, optargs):
        return self.ci.Action(action, **optargs)

    @vzdbus.method(dbus_interface=interface, in_signature="sa{sv}", out_signature="s")
    def Add(self, wtype, optargs):
        return self.ci.Add(wtype, **optargs)

    @vzdbus.method(dbus_interface=interface, in_signature="sssa{sv}")
    ...
    @vzdbus.method(dbus_interface=interface, in_signature="ssa{sv}", out_signature="s")
    def CloneWidget(self, widget, newparent, optargs):
        return self.ci.CloneWidget(str(widget), str(newparent), **optargs)

    @vzdbus.method(dbus_interface=interface, in_signature="sssa{sv}")
    def CreateHistogram(self, inexpr, outbinsds, outvalsds, optargs):
        self.ci.CreateHistogram(str(inexpr), str(outbinsds), str(outvalsds), **optargs)

    @vzdbus.method(dbus_interface=interface, in_signature="sa{sv}a{sv}")
    ...
    @vzdbus.method(dbus_interface=interface, in_signature="s", out_signature="v")
    def Get(self, val):
        return self.ci.Get(val)

    @vzdbus.method(dbus_interface=interface, in_signature="s", out_signature="as")
    def GetChildren(self, where):
        return self.ci.GetChildren(where=where)

    @vzdbus.method(dbus_interface=interface, in_signature="s", out_signature="adadadad")
    ...
    @vzdbus.method(dbus_interface=interface, in_signature="s", out_signature="iiddddad")
    ...
    @vzdbus.method(dbus_interface=interface, in_signature="ssa{sv}")
    def ImportFile(self, filename, descriptor, optargs):
        self.ci.ImportFile(filename, descriptor, **optargs)

    @vzdbus.method(dbus_interface=interface, in_signature="sasa{sv}")
    ...
    def ImportString(self, descriptor, string, optargs):
        self.ci.ImportString(str(descriptor), str(string), **optargs)
    ...
    @vzdbus.method(dbus_interface=interface, in_signature="ss")
    ...
    @vzdbus.method(dbus_interface=interface, in_signature="s", out_signature="s")
    ...
    @vzdbus.method(dbus_interface=interface, in_signature="sv")
    ...
    @vzdbus.method(dbus_interface=interface, in_signature="sadadadad")
    ...
    @vzdbus.method(dbus_interface=interface, in_signature="ssssa{sv}")
    def SetData2DExpressionXYZ(self, name, xexpr, yexpr, zexpr, optargs):
        self.ci.SetData2DExpressionXYZ(str(name), xexpr, yexpr, zexpr, **optargs)

    @vzdbus.method(dbus_interface=interface, in_signature="s(ddd)(ddd)sa{sv}")
    def SetData2DXYFunc(self, name, xstep, ystep, expr, optargs):
        self.ci.SetData2DXYFunc(str(name), xstep, ystep, expr, **optargs)

    @vzdbus.method(dbus_interface=interface, in_signature="sadii(dd)(dd)")
    ...
    @vzdbus.method(dbus_interface=interface, in_signature="sas")
    ...
    @vzdbus.method(dbus_interface=interface, in_signature="ss", out_signature="as")
