...
def doBinning(
    ...
    hist, edges = N.histogram(sdata, weights=weights, range=(minval, maxval), bins=bins)
    ...
    if calcmode in {"stddev", "variance", "mean", "median", "min", "max"}:
        ...
        in {
            ...
            "density_scaled",
            "counts-cumulative",
            "counts-cumulative-reverse",
            "fraction-cumulative",
            "fraction-cumulative-reverse",
        }
        and weights is None
    ):
    ...
    if calcmode in {"fraction", "fraction-cumulative", "fraction-cumulative-reverse"}:
        invcts = 1 / len(data)
        hist = hist * invcts
        if perr is not None:
            perr *= invcts
...
class Histo(GenericPlotter):
    ...
    def addSettings(klass, s):
        ...
        s.add(
            setting.FloatOrAuto(
                "minval", "Auto", descr=_("Minimum of range"), usertext=_("Minimum")
            ),
            6,
        )
        s.add(
            setting.FloatOrAuto(
                "maxval", "Auto", descr=_("Maximum of range"), usertext=_("Maximum")
            ),
            7,
        )
        ...
        s.add(
            setting.FloatList(
                "manual", [], descr=_("Manual binning edges"), usertext=_("Manual")
            ),
            10,
        )
        ...
        s.add(
            PlotLine("Line", descr=_("Plot line"), usertext=_("Plot line")),
            pixmap="settings_plotline",
        )
        s.add(
            FillBrush1("Fill1", descr=_("Fill under"), usertext=_("Fill under")),
            pixmap="settings_plotfillbelow",
        )
        s.add(
            FillBrush2("Fill2", descr=_("Fill over"), usertext=_("Fill over")),
            pixmap="settings_plotfillabove",
        )
        s.add(
            PostLine("PostLine", descr=_("Post line"), usertext=_("Post line")),
    ...
    def drawKeySymbol(self, number, painter, x, y, width, height):
        ...
        for fill in s.Fill1, s.Fill2:
            if not fill.hide:
                ...
                path.addRect(qt.QRectF(qt.QPointF(x, yp), qt.QPointF(x + width, y2)))
    ...
    def dataDraw(self, painter, axes, posn, cliprect):
        ...
        if not s.PostLine.hide:
            ...
            if vert:
                qtloops.plotLinesToPainter(painter, pos, ctsplot, pos, zeros, cliprect)
            else:
                qtloops.plotLinesToPainter(painter, ctsplot, pos, zeros, pos, cliprect)
        ...
        if not s.MarkerLine.hide or not s.MarkerFill.hide:
            ...
            utils.plotMarkers(painter, xplt, yplt, s.marker, markersize, clip=cliprect)
