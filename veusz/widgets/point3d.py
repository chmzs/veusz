...
class Point3D(plotters3d.GenericPlotter3D):
    ...
    def addSettings(klass, s):
        ...
        s.add(
            ErrorLine3D(
                "Error", descr=_("Error bar settings"), usertext=_("Error bar")
            ),
    ...
    def dataDrawPointsLine(self, painter, cont, coord):
        ...
        if filled and not s.MarkerFill.hide:
            ...
            if color2d is not None and cmapname != "none":
                ...
                colorimg = utils.applyColorMap(
                    cmap, "linear", color2d, 0.0, 1.0, s.MarkerFill.transparency
                )
                markerfillprop.setRGBs(colorimg)

        if markerlineprop or markerfillprop:
            ptobj = threed.Points(
                coord[0], coord[1], coord[2], pointpath, markerlineprop, markerfillprop
            )
            if scalepts:
        ...
        if not s.PlotLine.hide:
            ...
            if color2d is not None and cmapname != "none":
                ...
                colorimg = utils.applyColorMap(
                    cmap, "linear", color2d, 0.0, 1.0, s.PlotLine.transparency
                )
