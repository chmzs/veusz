...
class _AxisLabels(threed.AxisLabels):
    ...
    def drawAxisLabel(self, painter, valign):
        ...
        if self.axislabelposn == 0.0:
            halign = -1
        elif self.axislabelposn == 1.0:
            halign = 1
        else:
            halign = 0
...
class MajorTick(setting.Line3D):
    ...
    def __init__(self, name, **args):
        ...
        self.add(
            setting.Float(
                "length", 20.0, descr=_("Length of major ticks"), usertext=_("Length")
            )
        )
        self.add(
            setting.Int(
...
class MinorTick(setting.Line3D):
    ...
    def __init__(self, name, **args):
        ...
        self.add(
            setting.Float(
                "length", 10, descr=_("Length of minor ticks"), usertext=_("Length")
            )
        )
        self.add(
            setting.Int(
...
class Axis3D(widget.Widget):
    ...
    def addSettings(klass, s):
        ...
        s.add(axis.AutoRange("autoRange", "next-tick"))
        s.add(
            setting.Choice(
                "mode",
        ...
        s.add(
            setting.Float(
                "otherPosition1",
                0.0,
                descr=_("Fractional position of axis in its perpendicular direction 1"),
        ...
        s.add(
            setting.Float(
                "otherPosition2",
                0.0,
                descr=_("Fractional position of axis in its perpendicular direction 2"),
    ...
    def userdescription(self):
        ...
        s = self.settings
        return "range %s to %s%s" % (str(s.min), str(s.max), ["", " (log)"][s.log])
    ...
    def addLabels(
        ...
        if axislabelprop.hide:
            ...
            axislabelposn = {"at-minimum": 0, "centre": 0.5, "at-maximum": 1}[
                axislabelprop.position
            ]
    ...
    def addAxisTicks(
        self, painter, cont, dirn, linecoords, tickprops, ticklabelsprop, tickvals
    ):
    ...
    def drawToObject(self, painter, painthelper):
        ...
        self.addAxisTicks(
            painter, cont, dirn, linecoords, s.MinorTicks, None, self.minortickscalc
        )

        self.addGridLines(
            painter, cont, dirn, linecoords, s.GridLines, self.majortickscalc
        )
        self.addGridLines(
            painter, cont, dirn, linecoords, s.MinorGridLines, self.minortickscalc
        )
