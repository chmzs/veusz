...
class Line(plotters.FreePlotter):
    ...
    def addSettings(klass, s):
        ...
        s.add(
            setting.ChoiceSwitch(
                ...
                "length-angle",
                descr=_(
                    "Provide line position and length,angle or first and second points"
                ),
                usertext=_("Mode"),
                formatting=False,
                showfn=klass.showOrHideSetn,
            ),
            0,
        )
        ...
        s.add(
            setting.DatasetExtended(
                "angle",
                [0.0],
                descr=_("List of angle of lines, dataset or expression (degrees)"),
                usertext=_("Angles"),
                formatting=False,
            ),
            5,
    ...
    def _computeLinesLengthAngle(self, posn, lengthscaling):
        ...
        if maxlen > 1:
            if len(xpos) == 1:
                xpos = itertools.cycle(xpos)
            if len(ypos) == 1:
                ypos = itertools.cycle(ypos)
            if len(length) == 1:
                length = itertools.cycle(length)
            if len(angle) == 1:
                angle = itertools.cycle(angle)
    ...
    def _computeLinesPointToPoint(self, posn):
        ...
        if maxlen > 1:
            ...
            if len(ypos) == 1:
                ypos = itertools.cycle(ypos)
            if len(xpos2) == 1:
                xpos2 = itertools.cycle(xpos2)
            if len(ypos2) == 1:
                ypos2 = itertools.cycle(ypos2)
        ...
        for v in zip(xpos, ypos, xpos2, ypos2):
            ...
            if N.all(N.isfinite(v)):
                ...
                angle = math.atan2(v[3] - v[1], v[2] - v[0]) / math.pi * 180.0
    ...
    def draw(self, posn, phelper, outerbounds=None):
        ...
        isnotdataset = not s.get("xPos").isDataset(d) and not s.get("yPos").isDataset(d)

        if s.mode == "length-angle":
            isnotdataset = (
                isnotdataset
                and not s.get("length").isDataset(d)
                and not s.get("angle").isDataset(d)
            )
        else:
            isnotdataset = (
                isnotdataset
                and not s.get("xPos2").isDataset(d)
                and not s.get("yPos2").isDataset(d)
            )
            ...
            for index, (x, y, l, a) in enumerate(lines):
                ...
                if isnotdataset:
                    cgi = controlgraph.ControlLine(
                        ...
                        x,
                        y,
                        x + l * math.cos(a / 180.0 * math.pi),
                        y + l * math.sin(a / 180.0 * math.pi),
                    )
                    cgi.index = index
                    cgi.widgetposn = posn
    ...
    def updateControlItem(self, cgi, pt1, pt2):
        ...
        if s.mode == "length-angle":
            ...
            def la(ptx, pty):
                ...
                angle = (
                    math.atan2(pty - pt1[1], ptx - pt1[0]) * 180.0 / math.pi
                ) % 360.0
                return length, angle
