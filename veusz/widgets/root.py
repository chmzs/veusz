...
class Root(widget.Widget):
    ...
    def makeStylesheet(self, compatlevel=0):
        ...
        stylesheet = setting.StyleSheet(
            descr=_("Master settings for document"), usertext=_("Style sheet")
        )
        s = self.settings
    ...
    def changeLocale(self):
        ...
        self.document.locale.setNumberOptions(
            qt.QLocale.NumberOption.OmitGroupSeparator
        )

    def changeColorTheme(self):
        """Change color theme used by document."""
        self.document.evaluate.colors.setColorTheme(self.settings.colorTheme)
