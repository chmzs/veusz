...
class PickInfo:
    ...
    def __init__(
        self, widget=None, graphpos=None, labels=None, coords=None, index=None
    ):
        self.widget = widget
    ...
    def __bool__(self):
        return bool(self.widget and self.graphpos and self.labels and self.coords)
...
class GenericPickable:
    ...
    def _pickSign(self, i):
        ...
        pi = i + 1
        while pi < len(self.xgraph) and not N.isfinite(
            self.xgraph[pi] + self.ygraph[pi]
        ):
            pi += 1
    ...
    def pickPoint(self, x0, y0, bounds, distance_direction):
        ...
        if distance_direction == "vertical":
            ...
            # programming error
            assert (
                distance_direction == "radial"
                or distance_direction == "vertical"
                or distance_direction == "horizontal"
            )

        # ignore points which are offgraph or not finite
        with N.errstate(invalid="ignore"):
            outofbounds = (
                (self.xgraph < bounds[0])
                | (self.xgraph > bounds[2])
                | (self.ygraph < bounds[1])
                | (self.ygraph > bounds[3])
                | ~N.isfinite(self.xgraph)
                | ~N.isfinite(self.ygraph)
            )
    ...
    def pickIndex(self, oldindex, direction, bounds):
        ...
        if oldindex.index is None:
            ...
            i = N.logical_not(
                N.logical_or(self.xvals < oldindex.ivar, self.xvals > oldindex.ivar)
            )
        ...
        while (
            i >= 0
            and i < len(self.xgraph)
            and (
                not N.isfinite(self.xgraph[i])
                or not N.isfinite(self.ygraph[i])
                or (
                    self.xgraph[i] < bounds[0]
                    or self.xgraph[i] > bounds[2]
                    or self.ygraph[i] < bounds[1]
                    or self.ygraph[i] > bounds[3]
                )
            )
        ):
            i += incr
