...
class ControlMarginBox:
    def __init__(
        self, widget, posn, maxposn, painthelper, ismovable=True, isresizable=True
    ):
    ...
    def setWidgetMargins(self):
        ...
        helper = document.PaintHelper(
            self.document, self.pagesize, scaling=self.cgscale, dpi=self.dpi
        )
...
class _GraphMarginBox(qt.QGraphicsItem):
    ...
    def __init__(self, parent, params):
        ...
        self.lines = [
            _EdgeLine(self, params, ismovable=params.ismovable) for i in range(4)
        ]
    ...
    def updateCornerPosns(self):
        ...
        # move corners
        for corner, (xindex, yindex) in zip(self.corners, self.mapcornertoposn):
...
class _GraphResizableBox(qt.QGraphicsItem):
    ...
    def __init__(self, parent, params):
        ...
        # lines connecting corners
        self.lines = [_EdgeLine(self, params, ismovable=True) for i in range(4)]
    ...
    def updateFromCorner(self, corner, event):
        ...
        if corner in self.corners:
            ...
            par.angle = round((angle * RAD2DEG + 90.0) % 360, 2)
    ...
    def updateCorners(self):
        ...
        for corn, (xd, yd) in zip(self.corners, ((-1, -1), (1, -1), (-1, 1), (1, 1))):
            dx, dy = xd * par.dims[0] * 0.5, yd * par.dims[1] * 0.5
            corn.setScaledPos(
                dx * c - dy * s + par.posn[0], dx * s + dy * c + par.posn[1]
            )
        ...
        for i, (ci1, ci2) in enumerate(((0, 1), (2, 0), (1, 3), (2, 3))):
            ...
            self.lines[i].setScaledLinePos(
                pos1[0], pos1[1], corn[ci2].scaledX(), corn[ci2].scaledY()
            )
...
class ControlMovableBox(ControlMarginBox):
    ...
    def __init__(self, widget, posn, painthelper, crosspos=None):
        ...
        self.deltacrosspos = (crosspos[0] - self.posn[0], crosspos[1] - self.posn[1])
    ...
    def updateCornerPosns(self):
        ...
        if hasattr(self, "cross"):
            ...
            self.cross.setScaledPos(
                par.deltacrosspos[0] + par.posn[0], par.deltacrosspos[1] + par.posn[1]
            )
...
class _GraphLine(qt.QGraphicsLineItem, _ScaledShape):
    ...
    def updateFromCorner(self, corner, event):
        ...
        c = (self.p0.scaledX(), self.p0.scaledY(), self.p1.scaledX(), self.p1.scaledY())
...
class ControlAxisLine:
    ...
    def __init__(
        self, widget, painthelper, direction, minpos, maxpos, axispos, maxposn
    ):
        self.widget = widget
    ...
    def moved(self):
        """Has axis moved?"""
        return (
            self.minpos != self.minorig
            or self.maxpos != self.maxorig
            or self.axisorigpos != self.axispos
        )
...
class _GraphAxisLine(qt.QGraphicsItem):
    ...
    def __init__(self, parent, params):
        ...
        self.params = params
        self.pts = [
            _ShapeCorner(self, params),
            _ShapeCorner(self, params),
            _AxisRange(self, params),
            _AxisRange(self, params),
        ]
    ...
    def updatePositions(self):
        ...
        for i, (i0, i1) in enumerate(idxs):
            self.lines[i].setScaledLine(
                points[i0].x(), points[i0].y(), points[i1].x(), points[i1].y()
            )
    ...
    def updateMovedCntrl(self, corner, event, mode):
        ...
        if mode == "xy":
            if (event.modifiers() & qt.Qt.KeyboardModifier.ControlModifier) == 0:
                ...
                deltaM = threed.rotate3M4(-delta.y() * DEG2RAD, -delta.x() * DEG2RAD, 0)
            else:
                # if control is pressed, rotate along z axis for y direction
                deltaM = threed.rotate3M4(-delta.x() * DEG2RAD, 0, -delta.y() * DEG2RAD)
        elif mode == "y":
