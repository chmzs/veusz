...
class AxisFunction(axis.Axis):
    ...
    def logError(self, ex):
        ...
        self.document.log(
            _("Error in axis-function (%s): '%s'") % (self.settings.function, str(ex))
        )
    ...
    def getFunction(self):
        ...
        if compiled is None:
            ...
            try:
                solveFunction(function, [0.0], mint=mint, maxt=maxt)
    ...
    def lookupAxis(self, axisname):
        ...
        while w:
            for c in w.children:
                if c.name == axisname and c.isaxis and c is not self:
                    return c
            w = w.parent
        return None
    ...
    def computePlottedRange(self, force=False):
        ...
        axis.Axis.computePlottedRange(self, force=force, overriderange=therange)
    ...
    def _updateLinkedAxis(self, bounds, fraccoords):
        ...
        pos = N.all(deltas >= 0.0)
        neg = N.all(deltas <= 0.0)
        ...
        # directly.
        if (
            link.settings.direction == self.settings.direction
    ...
    def _linearInterpolWarning(self, vals, xcoords, ycoords):
        ...
        if N.any(vals < xcoords[0]) or N.any(vals > xcoords[-1]):
            self.document.log(_("Warning: values exceed bounds in axis-function"))
    ...
    def _graphToPlotter(self, vals):
        ...
        if self.graphcoords is None:
            ...
            return self._linearInterpolWarning(vals, self.graphcoords, self.pixcoords)
