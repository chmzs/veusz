...
def _brushExtFillPathGradient(
    painter, extbrush, path, stroke=None, dataindex=0, axes=None, fill_bounds=None
):
...
def brushExtFillPath(
    ...
    if gradient_module.is_gradient_enabled(extbrush.get("Gradient")):
        _brushExtFillPathGradient(
            painter, extbrush, path, stroke, dataindex, axes, fill_bounds
        )
        return
    ...
    if style in _fillcnvt:
        ...
        if extbrush.transparency >= 0:
            if extbrush.transparency == 100:
                ...
                return
            color.setAlphaF((100 - extbrush.transparency) / 100.0)
        ...
        if not extbrush.backhide:
            # background brush
            color = extbrush.get("backcolor").color(painter, dataindex=dataindex)
            if extbrush.backtransparency > 0:
                color.setAlphaF((100 - extbrush.backtransparency) / 100.0)
        ...
        lstyle, dashpattern = extbrush.get("linestyle")._linecnvt[extbrush.linestyle]
