...
def cubehelix(start, rots, hue, gamma, nlev=64):
    ...
    angle = 2 * N.pi * (start / 3.0 + 1.0 + rots * fract)
    fract = fract**gamma
...
def applyScaling(data, mode, minval, maxval):
    ...
    if mode == "linear":
        # linear scaling
        data = (data - minval) * (1.0 / (maxval - minval))
        ...
        with N.errstate(invalid="ignore", divide="ignore"):
            invrange = 1.0 / (N.log(maxval) - N.log(minval))
...
def applyColorMap(cmap, scaling, datain, minval, maxval, trans, transimg=None):
    ...
    if trans != 0:
        cmap = cmap.copy()
        cmap[:, 3] = (cmap[:, 3].astype(N.float32) * (100 - trans) / 100.0).astype(
            N.intc
        )
...
def makeColorbarImage(
    minval, maxval, scaling, cmap, transparency, direction="horizontal", barsize=128
):
    ...
    img = applyColorMap(cmap, colorscaling, vals, minval, maxval, transparency)
