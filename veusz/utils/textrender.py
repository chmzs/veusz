...
class RenderState:
    ...
    def __init__(self, font, painter, x, y, alignhorz, actually_render=True):
        self.font = font
...
class PartFrac(Part):
    ...
    def render(self, state):
        ...
        painter.save()
        pen = painter.pen()
        painter.setPen(qt.QPen(painter.pen().brush(), state.getPixelsPerPt() * 0.5))
        painter.setPen(pen)

        painter.drawLine(
            qt.QPointF(initx, inity - height / 2.0),
            qt.QPointF(initx + max(self.widths), inity - height / 2.0),
        )
...
class PartHat(Part):
    ...
    def render(self, state):
        ...
        penw = state.getPixelsPerPt() * 0.5
        painter.setPen(qt.QPen(painter.pen().brush(), penw))
        hatheight = min((state.x - initx) / 2, height / 3.0)
        painter.drawLine(
            qt.QPointF(initx, state.y - height + penw),
    ...
    while i < length:
        ...
        if p == r"\\":
            ...
            if p in symbols:
                ...
                if (
                    numargs == 1
                    and len(partlist) > i + 1
                    and isinstance(partlist[i + 1], str)
                ):
                    ...
                    partargs = [
                        makePartTree(k) for k in partlist[i + 1 : i + numargs + 1]
                    ]

                if p == "^" or p == "_":
                    if len(itemlist) > 0 and (
                        isinstance(itemlist[-1], PartSubScript)
                        or isinstance(itemlist[-1], PartSuperScript)
                        or isinstance(itemlist[-1], PartMultiScript)
                    ):
                        ...
                        if isinstance(itemlist[-1], PartMultiScript):
                            ...
                            itemlist[-1] = PartMultiScript(
                                [itemlist[-1], klass(partargs)]
                            )
                    else:
                        itemlist.append(klass(partargs))
                else:
...
class _Renderer:
    ...
    def ensureInBox(
        self, minx=-32767, maxx=32767, miny=-32767, maxy=32767, extraspace=False
    ):
        ...
        if extraspace:
            ...
            l = FontMetrics(self.font, self.painter.device()).height() * 0.2
            miny += l
...
class _StdRenderer(_Renderer):
    ...
    def _getWidthHeight(self):
        ...
        state = RenderState(
            self.font, self.painter, 0, 0, self.alignhorz, actually_render=False
        )
    ...
    def render(self):
        ...
        state = RenderState(self.font, self.painter, self.xi, self.yi, self.alignhorz)
...
class _MmlRenderer(_Renderer):
    ...
    def _initText(self, text):
        ...
        rpaint.pixperpt = screendev.logicalDotsPerInchY() / 72.0
        rpaint.scaling = 1.0
        ...
        self.drawscale = self.painter.dpi / screendev.logicalDotsPerInchY() / upscale
