...
class ColorsRandomize(ToolsPlugin):
    ...
    description_full = _(
        "Randomize the colors used in plotting markers, lines or error bars. Random colors in hue, saturation and luminosity (HSV) are chosen between the two colors given."
    )

    def __init__(self):
        """Construct plugin."""
        self.fields = [
            field.FieldWidget("widget", descr=_("Start from widget"), default="/"),
            field.FieldBool("randxy", descr=_("Randomize xy plotters"), default=True),
            field.FieldBool(
                "randfunc", descr=_("Randomize function plotters"), default=True
            ),
            field.FieldColor(
                "color1", descr=_("Start of color range"), default="#404040"
            ),
            field.FieldColor(
                "color2", descr=_("End of color range"), default="#ff0004"
            ),
        ]
...
class ColorsSequenceCMap(ToolsPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            field.FieldWidget("widget", descr=_("Start from widget"), default="/"),
            field.FieldBool("colorxy", descr=_("Color xy plotters"), default=True),
            field.FieldBool(
                "colorfunc", descr=_("Color function plotters"), default=True
            ),
            field.FieldText("match", descr=_("Match")),
            field.FieldBool("invert", descr=_("Invert colormap"), default=False),
            field.FieldBool("randomize", descr=_("Randomize order"), default=False),
            field.FieldColormap("colormap", descr=_("Colormap"), default="grey"),
        ]
    ...
    def apply(self, ifc, fields):
        ...
        cvals = ifc.GetColormap(
            fields["colormap"], invert=fields["invert"], nvals=max(1, len(widgets))
        )
        ...
        for idx, widget in enumerate(widgets):
            if cvals[idx, 3] == 255:
                # opaque
                col = "#%02x%02x%02x" % (cvals[idx, 0], cvals[idx, 1], cvals[idx, 2])
            else:
                # with transparency
                col = "#%02x%02x%02x%02x" % (
                    cvals[idx, 0],
...
class ColorsSequence(ToolsPlugin):
    ...
    description_full = _(
        "Give new colors to each widget in a sequence between the two colors given."
    )
    ...
    def __init__(self):
        ...
        self.fields = [
            field.FieldWidget("widget", descr=_("Start from widget"), default="/"),
            field.FieldBool("randxy", descr=_("Color xy plotters"), default=True),
            field.FieldBool(
                "randfunc", descr=_("Color function plotters"), default=True
            ),
            field.FieldColor(
                "color1", descr=_("Start of color range"), default="#ff0000"
            ),
            field.FieldColor(
                "color2", descr=_("End of color range"), default="#4000ff"
            ),
        ]
    ...
    def apply(self, ifc, fields):
        ...
        numwidgets = len(list(fromwidget.WalkWidgets(widgettype="xy"))) + len(
            list(fromwidget.WalkWidgets(widgettype="function"))
        )
...
class ColorsReplace(ToolsPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            field.FieldWidget("widget", descr=_("Start from widget"), default="/"),
            field.FieldBool(
                "follow", descr=_("Change references and defaults"), default=True
            ),
            field.FieldColor("color1", descr=_("Color to change"), default="black"),
            field.FieldColor("color2", descr=_("Replacement color"), default="red"),
        ]
...
class ColorsSwap(ToolsPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldColor("color1", descr=_("First color"), default="black"),
            field.FieldColor("color2", descr=_("Second color"), default="red"),
        ]
...
class TextReplace(ToolsPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldBool(
                "onlystr", descr=_("Change only textual data"), default=False
            ),
            field.FieldText("text1", descr=_("Text to change"), default=""),
            field.FieldText("text2", descr=_("Replacement text"), default=""),
        ]
...
class WidgetsClone(ToolsPlugin):
    ...
    description_full = _(
        'Take a widget and its children and clone it, plotting different sets of data in each clone.\nHint: Use a "*" in the name of a replacement dataset to match multiple datasets, e.g. x_*'
    )

    def __init__(self):
        """Construct plugin."""
        self.fields = [
            field.FieldWidget("widget", descr=_("Clone widget"), default=""),
            field.FieldDataset("ds1", descr=_("Dataset 1 to change"), default=""),
            field.FieldDatasetMulti("ds1repl", descr=_("Replacements for dataset 1")),
            field.FieldDataset(
                "ds2", descr=_("Dataset 2 to change (optional)"), default=""
            ),
            field.FieldDatasetMulti("ds2repl", descr=_("Replacements for dataset 2")),
            field.FieldBool(
                "names", descr=_("Build new names from datasets"), default=True
            ),
        ]
...
class FontSize(ToolsPlugin):
    ...
    def __init__(self, dirn):
        ...
        self.fields = [
            ...
            field.FieldFloat("delta", descr=_("Change by value"), default=2),
        ]
...
class GridGraphLabels(ToolsPlugin):
    ...
    def __init__(self):
        self.fields = [
            field.FieldWidget(
                "grid",
                descr=_("Grid widget containing graphs"),
                default="/",
                widgettypes={"grid"},
            ),
            field.FieldCombo(
                "label_type",
                _("Label sequence type"),
                default="lowercase",
                items=("lowercase", "uppercase", "numbers"),
                editable=False,
            ),
            field.FieldText(
                "prefix",
            ...
            field.FieldFloat(
                "x_pos",
                descr=_("X position (0-1, left to right)"),
                default=0.1,
                minval=0.0,
                maxval=1.0,
            ),
            field.FieldFloat(
                "y_pos",
                descr=_("Y position (0-1, bottom to top)"),
                default=0.9,
                minval=0.0,
                maxval=1.0,
            ),
            ...
            field.FieldCombo(
                "valign",
                _("Vertical text alignment"),
                default="top",
                items=("bottom", "centre", "top"),
                editable=False,
            ),
            field.FieldDistance(
    ...
    def apply(self, ifc, fields):
        ...
        grid_path = fields["grid"]
        grid_widget = ifc.Root.fromPath(grid_path)

        # Check widgettype (string) instead of isinstance against class
        if grid_widget.widgettype != "grid":
            raise ToolsPluginException(
                _("Selected widget is not a Grid (got: %(type)s at %(path)s)")
                % {"type": grid_widget.widgettype, "path": grid_path}
            )

        # Find all Graph children in the grid using WalkWidgets
        graphs = list(grid_widget.WalkWidgets(widgettype="graph"))
