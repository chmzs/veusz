...
class DatasetPluginHelper:
    ...
    def datasets1d(self):
        ...
        return [
            name
            for name, ds in self._doc.data.items()
    ...
    def datasets2d(self):
        ...
        return [
            name
            for name, ds in self._doc.data.items()
            if (ds.dimensions == 2 and ds.datatype == "numeric")
        ]
    ...
    def datasetstext(self):
        ...
        return [
            name
            for name, ds in self._doc.data.items()
            if (ds.dimensions == 1 and ds.datatype == "text")
        ]
    ...
    def getDataset(self, name, dimensions=1):
        ...
        if ds.dimensions != dimensions and dimensions != "all":
            raise DatasetPluginException(
                _("Dataset '%s' does not have %s dimensions") % (name, dimensions)
            )
        ...
        if isinstance(ds, datasets.DatasetDateTime):
            ...
            return Dataset1D(
                name, data=ds.data, serr=ds.serr, perr=ds.perr, nerr=ds.nerr
            )
...
class MultiplyDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _("Multiply a dataset by a factor. Error bars are also scaled.")

    def __init__(self):
        """Define fields."""
        self.fields = [
            field.FieldDataset("ds_in", _("Input dataset")),
            field.FieldFloat("factor", _("Factor"), default=1.0),
...
class AddDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _("Add a dataset by adding a value. Error bars remain the same.")

    def __init__(self):
        """Define fields."""
        self.fields = [
            field.FieldDataset("ds_in", _("Input dataset")),
            field.FieldFloat("value", _("Add value"), default=0.0),
            field.FieldDataset("ds_out", _("Output dataset name")),
        ]
...
class InterleaveDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        def interleave(datasets):
            ...
            # stick in columns
            ds = [N.hstack((d, N.zeros(maxlength - len(d)))) for d in datasets]
            # which elements are valid
            good = [
                N.hstack(
                    (
                        N.ones(len(d), dtype=N.bool_),
...
class ThinDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        "Select data points at intervals from dataset to create new dataset"
    )
...
class AddDatasetsPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        "Add datasets together to make a single dataset. Error bars are combined."
    )
...
class SubtractMeanDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        "Subtract mean from dataset, optionally dividing by standard deviation."
    )
    ...
    def updateDatasets(self, fields, helper):
        ...
        self.dsout.update(data=vals, serr=dsin.serr, perr=dsin.perr, nerr=dsin.nerr)
...
class SubtractMinimumDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        self.dsout.update(data=data, serr=dsin.serr, perr=dsin.perr, nerr=dsin.nerr)
...
class MultiplyDatasetsPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        "Multiply two or more datasets. Combined error bars are also calculated."
    )
...
class DivideDatasetsPlugin(_OneOutputDatasetPlugin):
    ...
    description_short = _("Compute ratio or fractional difference between two datasets")
    description_full = _("Divide or compute fractional difference between two datasets")
    ...
    def __init__(self):
        ...
        self.fields = [
            field.FieldDataset("ds_in1", _("Input dataset 1")),
            field.FieldDataset("ds_in2", _("Input dataset 2")),
            field.FieldBool("frac", _("Compute fractional difference"), default=False),
            field.FieldDataset("ds_out", _("Output dataset name")),
        ]
...
class DivideNormalizePlugin(_OneOutputDatasetPlugin):
    ...
    name = "Normalize"
    description_short = description_full = _("Divide dataset by its sum of values")
...
class ExtremesDatasetPlugin(DatasetPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldDataset(
                "ds_errorbar", _("Output range as error bars in dataset (optional)")
            ),
        ]
...
class CumulativePlugin(_OneOutputDatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        def cumsum(v):
            ...
            if fromend:
                v = v[::-1]
            v[N.logical_not(N.isfinite(v))] = 0.0
            c = N.cumsum(v)
            if fromend:
                c = c[::-1]
            return c
...
class FilterDatasetPlugin(_OneOutputDatasetPlugin):
    ...
    description_full = _(
        'Filter a dataset using an expression, e.g. "x>10" or "(x>1) & (y<2)"'
    )
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldBool(
                "replacenan",
                _("Replace excluded points by NaN\n(indicate missing points)"),
                default=False,
            ),
            field.FieldDataset("ds_out", _("Output dataset")),
        ]
    ...
    def updateDatasets(self, fields, helper):
        ...
        try:
            ...
            raise DatasetPluginException(_("Error filtering dataset: '%s')") % str(e))
...
class MovingAveragePlugin(_OneOutputDatasetPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldInt(
                "width",
                _("Points either side of point to average"),
                default=1,
                minval=0,
            ),
            field.FieldBool("weighterrors", _("Weight by error bars"), default=True),
            field.FieldDataset("ds_out", _("Output dataset")),
        ]
    ...
    def updateDatasets(self, fields, helper):
        ...
        if fields["weighterrors"]:
            if ds_in.serr is not None:
                weights = 1.0 / ds_in.serr**2
            elif ds_in.perr is not None and ds_in.nerr is not None:
                weights = 1.0 / ((ds_in.perr**2 + ds_in.nerr**2) / 2.0)
        width = fields["width"]
...
class LinearInterpolatePlugin(_OneOutputDatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        interpol = N.interp(
            ds_xprime, ds_x[:minlenin], ds_y[:minlenin], left=pad, right=pad
        )
...
class ReBinXYPlugin(DatasetPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldInt("binsize", _("Bin size (N)"), minval=1, default=2),
            field.FieldCombo(
                "mode", _("Mode of binning"), items=("sum", "average"), editable=False
            ),
        ...
        if len(self.dssout) == 2:
            ...
            if dsx.serr is not None:
                ...
                if len(dsx.data) > 2:
                    ...
                    minvals = N.insert(
                        minvals, 0, dsx.data[0] - 0.5 * (dsx.data[1] - dsx.data[0])
                    )
                elif len(dsx.data) != 0:
            ...
            if dsx.serr is not None:
                ...
                if len(dsx.data) > 2:
                    maxvals = N.append(
                        maxvals, dsx.data[-1] + 0.5 * (dsx.data[-1] - dsx.data[-2])
                    )
                elif len(dsx.data) != 0:
            ...
            self.dssout[1].update(
                data=0.5 * (minbin + maxbin), serr=0.5 * (maxbin - minbin)
            )
...
class Histogram2D(DatasetPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            ...
            field.FieldInt(
                "binsx", _("Number of bins for dataset x"), default=10, minval=2
            ),
            ...
            field.FieldInt(
                "binsy", _("Number of bins for dataset y"), default=10, minval=2
            ),
            field.FieldCombo(
                "mode",
                _("Mode"),
    ...
    def updateDatasets(self, fields, helper):
        ...
        histo, xedge, yedge = N.histogram2d(
            dsy,
            dsx,
            bins=[fields["binsy"], fields["binsx"]],
...
class ConvertNumbersToText(DatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        data = [utils.formatNumber(n, f, locale=helper.locale) for n in ds_in.data]
...
class ClipPlugin(_OneOutputDatasetPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            field.FieldDataset("ds_in", _("Input dataset")),
            field.FieldFloat("minimum", _("Minimum"), default=0.0),
            field.FieldBool("disablemin", _("Disable minimum")),
            field.FieldFloat("maximum", _("Maximum"), default=1.0),
...
class LogPlugin(_OneOutputDatasetPlugin):
    ...
    def __init__(self):
        ...
        self.fields = [
            field.FieldDataset("ds_in", _("Input dataset")),
            field.FieldFloat("base", _("Base"), default=10.0, minval=1e-10),
            field.FieldDataset("ds_out", _("Output dataset")),
        ]
    ...
    def updateDatasets(self, fields, helper):
        ...
        invlogbase = 1.0 / N.log(fields["base"])
...
class ReshapeDatasetPlugin(DatasetPlugin):
    ...
    def updateDatasets(self, fields, helper):
        ...
        try:
            ...
            raise DatasetPluginException("Could not interpret shape text")
        try:
            data = ds_in.data.reshape(shape)
        except ValueError:
            raise DatasetPluginException("Cannot reshape: wrong number of dimensions")
