...
from .base import DatasetConcreteBase, DatasetException, DatasetExpressionException
...
class Dataset2DBase(DatasetConcreteBase):
    ...
    def isLinearImage(self):
        ...
        return (
            self.xedge is None
            and self.yedge is None
            and self.xcent is None
            and self.ycent is None
        )
    ...
    def getPixelEdges(self, scalefnx=None, scalefny=None):
        ...
        if self.xedge is not None:
            ...
            xg = N.linspace(self.xrange[0], self.xrange[1], self.data.shape[1] + 1)
            if scalefnx:
                xg = scalefnx(xg)
        ...
        if self.yedge is not None:
            ...
            yg = N.linspace(self.yrange[0], self.yrange[1], self.data.shape[0] + 1)
            if scalefny:
                yg = scalefny(yg)
    ...
    def getPixelCentres(self):
        ...
        if self.xcent is not None:
            ...
            try:
                xc = (N.arange(xw) + 0.5) * (
                    (self.xrange[1] - self.xrange[0]) / xw
                ) + self.xrange[0]
        ...
        if self.ycent is not None:
            ...
            try:
                yc = (N.arange(yw) + 0.5) * (
                    (self.yrange[1] - self.yrange[0]) / yw
                ) + self.yrange[0]
            except ZeroDivisionError:
                yc = 0.0
...
class Dataset2D(Dataset2DBase):
    ...
    def saveDataDumpToHDF5(self, group, name):
        ...
        for v in ("data", "xcent", "xedge", "ycent", "yedge", "xrange", "yrange"):
...
class Dataset2DXYFunc(Dataset2DBase):
    ...
    def evalDataset(self):
        ...
        xarange = N.arange(self.xstep[0], self.xstep[1] + self.xstep[2], self.xstep[2])
        yarange = N.arange(self.ystep[0], self.ystep[1] + self.ystep[2], self.ystep[2])
